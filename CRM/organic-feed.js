var _klyAds = parent.kmklabs || parent.kly;
    ...
    var _anchorThumb = (_platform == "mobile")  ? null : _organicFeed.querySelectorAll(".articles--iridescent-list--text-item__figure-thumbnail-link");
    ...
    var _labelCategory = (_platform == "mobile")  ? _organicFeed.querySelectorAll(".article-snippet__label") : _organicFeed.querySelectorAll(".articles--iridescent-list--text-item__category");
    ...
    function _injectAds() {
        ...
        _replaceAttribute(_anchorThumb, "href", _landingPage);
        _replaceAttribute(_anchorThumb, "target", "_blank");
        _replaceAttribute(_anchorThumb, "title", _title);

        //replace label category
        _replaceAttribute(_labelCategory,'text',_label);
        if(_platform !== "mobile"){
            _replaceAttribute(_labelCategory,'href',_labelURL);
        }
    ...
    function getTextTypeArticle( article ){
        ...
        try {
            article.forEach((val,key)=>{
                if (val.getAttribute('data-type') === "TextTypeArticle" && ( val.getAttribute('data-template-name') === "article-snippet" || val.getAttribute('data-template-name') === "article-iridescent-list-text") ) {
                    _result = article[key];
                    throw _break;
                }
            })
