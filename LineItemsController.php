...
use Google\AdsApi\AdManager\v202211\AdUnitTargeting as V202211AdUnitTargeting;
use Google\AdsApi\AdManager\v202211\CostType as V202211CostType;
use Google\AdsApi\AdManager\v202211\CreativePlaceholder;
use Google\AdsApi\AdManager\v202211\CreativeRotationType;
use Google\AdsApi\AdManager\v202211\CustomCriteria as V202211CustomCriteria;
use Google\AdsApi\AdManager\v202211\CustomCriteriaSet as V202211CustomCriteriaSet;
use Google\AdsApi\AdManager\v202211\Goal;
use Google\AdsApi\AdManager\v202211\GoalType;
use Google\AdsApi\AdManager\v202211\InventoryTargeting as V202211InventoryTargeting;
...
use Google\AdsApi\AdManager\v202211\Money as V202211Money;
use Google\AdsApi\AdManager\v202211\UnarchiveLineItems;
use Google\AdsApi\AdManager\v202211\ResumeLineItems;
use Google\AdsApi\AdManager\v202211\Size;
use Google\AdsApi\AdManager\v202211\StartDateTimeType;
use Google\AdsApi\AdManager\v202211\Targeting as V202211Targeting;
...
class LineItemsController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->creative_list = [
            ...
            "yahoossp" => 
                    [
                        "sizes" => [
                            "300x600" => 138410348203,
                            "300x250" => 138409766009,
                            "320x100" => 138410347672,
                            "320x50" => 138410350387,
                            "970x90" => 138409848480,
                            "728x90" => 138409766285,
                            "970x250" => 138409848162,
                        ],
                        "keyId" => 13675063 /** kay-values */
                    ],
            "taboola" => 
                    [
                        "sizes" => [
                            "300x600" => 138412753101,
                            "300x250" => 138413238178,
                            "320x100" => 138413238181,
                            "320x50" => 138413238187,
                            "970x90" => 138412753104,
                            "728x90" => 138412753098,
                            "970x250" => 138413238184,
                        ],
                        "keyId" => 13712857 /** kay-values */
                    ],
            "triplelift" => 
                    [
                        "sizes" => [
                            "300x600" => 138419185108,
                            "300x250" => 138419185105,
                            "320x100" => 138418557695,
                            "320x50" => 138419184172,
                            "970x90" => 138418554762,
                            "728x90" => 138419175745,
                            "970x250" => 138418557692,
                        ],
                        "keyId" => 13779755 /** kay-values */
                    ]
    ...
    public function create(Request $request)
    {
        ...
        // $this->lineitemsGeneratorNEW($orderId, $bidder_code);
        
        // $this->lineitemsGeneratorAdded($orderId, $bidder_code);
        //$this->lineitemsGeneratorCheck($orderId, $bidder_code);
        // $this->lineitemUpdaterTargeting($orderId,$bidder_code);
        $this->lineitemsGeneratorDefault($orderId, $bidder_code);/** DEFAULT GENERATOR */
        
    }

    protected function lineitemUpdaterTargeting($orderId,$bidder_code)
    {
        ...
        $excludeLineItems = array("5727319101","5727319173","5727319353","5727319404","5727337188","6044642053");//"6043245531");//"5975530512","5975530551","5975530734","5976845479","5976845740","5977470842","5977471187","5977471382","5977472345","5977472393");
		$price = [];
        foreach ($lineItems as $key=>$updatedLineItem) {
            $liName = $updatedLineItem->getName();
            $subj = explode("$",$liName);
            preg_match('!\b[0-9.]{1,}\b!',$subj[1],$arrPrice);
            $strPrice = $arrPrice[0];
            $liStatus = $updatedLineItem->getStatus();
            $price[] = $strPrice;
            // echo "<pre>";
            // echo $bidder_code . " | " . $strPrice . " | " . $liStatus . " | " . $updatedLineItem->getId() . " | ";
            // print_r($liName);
            // echo "</pre>";
            // if( $liStatus === 'PAUSED'){
            //     echo $updatedLineItem->getId() . "</br>";
            // }
            
            // if( $this->proceed && $liStatus === 'PAUSED' && !in_array($updatedLineItem->getId(), $excludeLineItems) ){
            //     $this->updateLineItemCustomTargetting($lineItemService,$updatedLineItem,$bidder_code,$strPrice);
            // }
		}
        sort($price);
        echo "<pre>";
    ...
    protected function updateLineItemCustomTargetting(
        $lineItemService,
        $updatedLineItem,
        $bCode,
        $strPrice
    ) 
    {
        ...
        // $keyIdExc = 11742033;	
        // $valIdExc = [448561936271,448561949819];

        $keyIdPrc = $this->creative_list[$bCode]["keyId"];
        $valIdPrc = $this->getKey($keyIdPrc ,$strPrice);

        $customCriteriaChildPrice = new V202211CustomCriteria();
        $customCriteriaChildPrice->setKeyId($keyIdPrc); // key targetting IDs
        $customCriteriaChildPrice->setValueIds([$valIdPrc]); // values targetting IDs
        $customCriteriaChildPrice->setOperator('IS');

		//create custom targetting
        // $customCriteriaChildExclude = new V202211CustomCriteria();
        // $customCriteriaChildExclude->setKeyId($keyIdExc); // key targetting IDs
        // $customCriteriaChildExclude->setValueIds($valIdExc); // values targetting IDs
        // $customCriteriaChildExclude->setOperator('IS_NOT');
         // $customTargettingChild->setChildren([$customCriteriaChildPrice, $customCriteriaChildExclude]);

        $customTargettingChild = new V202211CustomCriteriaSet();
        $customTargettingChild->setLogicalOperator('AND');
        $customTargettingChild->setChildren([$customCriteriaChildPrice]);

		$adunitTargeting = new V202211AdUnitTargeting();
        $adunitTargeting->setAdunitId(35505050);
		
		// Create inventory targeting.
        $inventoryTargeting = new V202211InventoryTargeting();
        $inventoryTargeting->setTargetedAdUnits([$adunitTargeting]);
		
		// Create targeting.
        $targeting = new V202211Targeting();
		$targeting->setInventoryTargeting($inventoryTargeting);
        $targeting->setCustomTargeting($customTargettingChild);
		...
		try {
            ...
            if ($resultResume !== null) {
				 echo $results[0]->getId() . " Resume !<br/>";
            } else {
    ...
    ) 
    {
        ...
        $lineitemSubject = 'KLY Prebid Header Bidding - ' . $bCode . ' ' . $ext .' - $' . $strPrice;
		...
		//create custom targetting
        $customCriteriaChild = new V202211CustomCriteria();
        ...
        $customTargetting = new V202211CustomCriteriaSet();
		...
		$lineItem->setTargeting($targeting);
		$lineItem->setCostType(V202211CostType::CPM);
        $lineItem->setCostPerUnit(new V202211Money('USD', $micronPrice));
    ...
    public function lineitemsGeneratorNEW($orderId, $bidder_code)
    {   
        $lineItemService = $this->serviceFactory->createLineItemService($this->session);
		$statementBuilder = new V202211StatementBuilder();
		// $statementBuilder->Where('orderId = :orderId AND isArchived = 1');
        ...
        $lineItems = $page->getResults();
        foreach ($lineItems  as $key => $updatedLineItem ) {
                echo $key . " | " . $updatedLineItem->getId() . " | " . $updatedLineItem->getName() . "<br/>";
        }
        // $unarchivedLineitem = 1;
        // if($this->proceed ){
        //     $action = new UnarchiveLineItems();
        //     $unarchivedLineitem = $lineItemService->performlineItemAction(
        //         $action,
        //         $statementBuilder->toStatement()
        //     );
        // }
        
        // if ($unarchivedLineitem !== null) {
        //     // foreach ($lineItems  as $key => $updatedLineItem ) {
        //     //     echo $key . " | " . $updatedLineItem->getId() . " | " . $updatedLineItem->getName() . "<br/>";
        //     // }
        ...
        //     //     echo $count ." | ". $key . " | " . $updatedLineItem->getId() . " | " . $updatedLineItem->getName() . " | " . $strPrice_rate  . "<br/>";
        ...
        //     if($this->proceed ){
        //         if(($lineitemId = $this->generateLineitem($orderId, $strPrice_rate, $bidder_code, "24102022")) !== FALSE)
    ...
    /** DEFAULT GENERATOR */
    public function lineitemsGeneratorDefault($orderId, $bidder_code)
    {
    ...
    protected function generateLineitem($orderId, $strPrice, $bCode, $ext = "" )
    {
        $lineItemService = $this->serviceFactory->createLineItemService($this->session);
        $micronPrice = floatval($strPrice) * 1000000;
        $lineitemSubject = 'KLY Prebid Header Bidding - ' . $bCode . ' ' . $ext . ' - $' . $strPrice;
    ...
    protected function updateLineItem2(
        $lineItemService,
        $updatedLineItem,
		$strPrice, 
		$bCode
